...
function SP:handlePlate(plateFrame, plateData)
	...
	bars.cast:HookScript("OnShow", function(self) SP:OnShowCastbar(self, regions) end)
...
function SP:proccessBlizzBar(plateFrame, setVisible)
	...
	local bars, regions = {}, {}
		...
		if wowbuild >= 20426 then --6.2.2 ONLY START

			health = plateFrame.ArtContainer.HealthBar
			cast = plateFrame.ArtContainer.CastBar

			health.parentPlate = plateFrame
			cast.parentPlate = plateFrame

			regions.healthborder = plateFrame.ArtContainer.Border
			regions.highlight = plateFrame.ArtContainer.Highlight
			regions.level = plateFrame.ArtContainer.LevelText
			regions.raidicon = plateFrame.ArtContainer.RaidTargetIcon
			regions.eliteicon = plateFrame.ArtContainer.EliteIcon
			regions.threatglow = plateFrame.ArtContainer.AggroWarningTexture
			regions.skullicon = plateFrame.ArtContainer.HighLevelIcon
			regions.name = plateFrame.NameContainer.NameText

			regions.castborder = plateFrame.ArtContainer.CastBarBorder
			regions.spellicon = plateFrame.ArtContainer.CastBarSpellIcon
			regions.castnostop = plateFrame.ArtContainer.CastBarFrameShield
			regions.spelltext = plateFrame.ArtContainer.CastBarText
			regions.spellshadow = plateFrame.ArtContainer.CastBarTextBG
			...
			barFrame,
			nameFrame 
				= plateFrame:GetChildren()

			health, 
			cast
				= barFrame:GetChildren()
		...
		if setVisible then
			...
			regions.raidicon:ClearAllPoints()
			regions.raidicon:SetPoint("TOP", plateFrame, 0, 40)
...
function SP:OnShowCastbar(bar, regions)
	...
	cast.name:SetText(regions.spelltext:GetText())
	cast:Show()
...
function SP:getPreDefinedPlate(plate, name)
	...
	frame.cast.name = frame.cast.bar:CreateFontString(nil, "OVERLAY")
	frame.cast.name:SetPoint("LEFT", frame.cast, 8, 0)
	frame.cast.name:SetFont(PURISTA_FONT, 10, "OUTLINE")
	frame.cast.name:SetJustifyH("LEFT")
	frame.cast.name:SetShadowOffset(1, -1)
	frame.cast.name:SetTextColor(1, 1, 1)
	frame.cast.name:SetText("Lichtblitz")
...
function SP:CCFound( destGUID, sourceGUID, unitID, icon, expires, spellID, isStunn )
	...
	if not arenaPlates[unitID] then return end

	-- expires - GetTime()
	if isStunn then

		if not activeStunns[unitID] or activeStunns[unitID].expires < expires then
